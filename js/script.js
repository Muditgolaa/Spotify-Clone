...
async function displayAlbums() {
    ...
    let array = Array.from(anchors)
        ...
        if (decoded.includes("\\songs\\")) {
            let folder = decoded.split("\\").pop().replace("/", "");
            ...
            cardContainer.innerHTML = cardContainer.innerHTML + `<div data-folder="${folder}" class="card">
                        <div class="play">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"
                                color="currentColor" fill="bg-black" stroke="#141B34" stroke-width="1.5"
...
async function main() {
    ...
    previous.addEventListener("click", () => {
        let current = decodeURIComponent(currentSong.src.split("/").slice(-1)[0])
    ...
    // add an eventlistener to mute volume
